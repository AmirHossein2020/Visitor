...
export default function OrderDetailPage({ orderId }) {
  ...
  const load = useCallback(() => getOrder(orderId).then(setOrder).catch((requestError) => setError(getErrorMessage(requestError))), [orderId]);
  useEffect(() => { load(); }, [load]);
    ...
    <OrderLayout title="جزئیات سفارش">
      ...
      {!order ? <p className="py-12 text-center text-slate-600">در حال دریافت سفارش…</p> : <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_340px]">
        <section className="space-y-3">
          <div className="rounded-2xl bg-white p-5 ring-1 ring-slate-200"><p className="text-sm text-slate-500">مشتری</p><h2 className="mt-1 text-xl font-bold">{order.customer_name}</h2><p className="mt-2 text-sm text-slate-500">وضعیت: {order.status_display}</p></div>
          {order.items.map((item) => <article className="rounded-2xl bg-white p-4 ring-1 ring-slate-200" key={item.id}>
            <div className="flex justify-between gap-3"><div><h3 className="font-bold">{item.product_name_snapshot}</h3><p className="text-sm text-slate-500">{item.brand_snapshot || "بدون برند"} · {item.unit_snapshot}</p></div><span className="font-semibold text-teal-800">{formatPrice(item.line_total)} تومان</span></div>
        ...
        <aside className="space-y-4">
          ...
          {order.notes && <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200"><p className="text-sm text-slate-500">یادداشت</p><p className="mt-2 whitespace-pre-wrap">{order.notes}</p></div>}
