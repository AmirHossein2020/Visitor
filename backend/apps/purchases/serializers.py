...
class PurchaseItemSerializer(serializers.ModelSerializer):
    class Meta:
        model = PurchaseItem
        fields = ("id", "product", "product_name_snapshot", "brand_snapshot", "unit_snapshot", "quantity", "unit_price", "line_total", "created_at", "updated_at")
...
class PurchaseListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Purchase
        fields = ("id", "customer", "customer_name", "status", "status_display", "purchase_date", "total_amount", "created_at", "updated_at")
        read_only_fields = ("id", "customer_name", "status_display", "total_amount", "created_at", "updated_at")


class PurchaseSerializer(PurchaseListSerializer):
    items = PurchaseItemSerializer(many=True, read_only=True)

    class Meta(PurchaseListSerializer.Meta):
        fields = PurchaseListSerializer.Meta.fields + ("notes", "items")
...
class UpdatePurchaseItemSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        if not attrs:
            raise serializers.ValidationError("مقدار یا قیمت خرید را وارد کنید.")
        return attrs
